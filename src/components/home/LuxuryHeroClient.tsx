...
export function LuxuryHeroClient({ floatingPackages }: { floatingPackages: TourPackage[] }) {
  ...
  // Armenia Travel–style: inset hero expands to full-bleed while sticky on scroll
  useEffect(() => {
    ...
    if (reduced.matches) {
      section.style.setProperty("--hero-expand", "0");
      return;
    }

    // Short distance = snappy expand; light lerp = smooth without lag
    const EXPAND_PX = 260;
    let current = 0;
    let target = 0;
    let raf = 0;
    ...
    const readTarget = () => {
      const scrolled = Math.min(Math.max(-section.getBoundingClientRect().top, 0), EXPAND_PX);
      target = easeOutCubic(scrolled / EXPAND_PX);
    };

    const tick = () => {
      current += (target - current) * 0.35;
      if (Math.abs(target - current) < 0.001) current = target;
      section.style.setProperty("--hero-expand", current.toFixed(4));
    ...
    readTarget();
    section.style.setProperty("--hero-expand", target.toFixed(4));
    current = target;
  ...
  return (
    <section
      ref={sectionRef}
      className="relative z-10 h-[calc(100svh+280px)] [--hero-expand:0] [--hero-inset:0.75rem] sm:[--hero-inset:1.25rem] lg:[--hero-inset:2rem]"
    >
      <div className="sticky top-0">
        <div
          className="w-full"
          style={{
            paddingLeft: "calc(var(--hero-inset) * (1 - var(--hero-expand)))",
            paddingRight: "calc(var(--hero-inset) * (1 - var(--hero-expand)))",
            paddingBottom: "calc(1.5rem * (1 - var(--hero-expand)))",
          }}
        >
          <div
            className="relative mx-auto overflow-hidden"
            style={{
              maxWidth: "calc(1776px + (100% - 1776px) * var(--hero-expand))",
              borderRadius: "calc(1.5rem * (1 - var(--hero-expand)))",
              height:
                "calc(min(780px, 92vh) + (100svh - min(780px, 92vh)) * var(--hero-expand))",
            }}
          >
            {/* Background slider */}
            ...
            <a
              ...
              style={{ opacity: "calc(1 - var(--hero-expand))" }}
            >
